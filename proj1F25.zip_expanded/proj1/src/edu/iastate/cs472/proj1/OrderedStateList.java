...
public class OrderedStateList 
{
	 ...
	 */
	  ...
	  public OrderedStateList(Heuristic h, boolean isOpen)
	  {
		  ...
		  isOPEN = isOpen;
		  int[][] placeholder = {{1, 2, 3},{8, 0, 4},{7, 6, 5}};
		  head = new State(placeholder); // Dummy node
		  // previous and next point to itself
		  head.next = head;
		  head.previous = head;
		  size = 0;
	  }
	  ...
	  public void addState(State s)
	  {
		...
		while(current != head && compareStates(s, current) > 0){ // Traverse the list to find the correct position
			current = current.next;
		}
		insertBefore(s, current); // Insert s before the found position
	  }

	  /*
	   * Insert s1 before s2 in the list.  Adjust the next and previous fields of s1, s2, and
	   * their neighbors accordingly.  Increment the instance variable size.
	   */
	  private void insertBefore(State s1, State s2){
		  s1.next = s2;
	  ...
	  public State findState(State s)
	  {
		  State current = head.next;
		  while(current != head){
			  if(current.equals(s)){ // Return if we found the state
				  return current;
			  }
			  current = current.next;
		  }
	  ...
	  public State remove()
	  {
		  ...
		  State first = head.next; // Get the first state
		  removeState(first); // Remove it from the list
		  return first;
	  }
	  ...
	  {
		  if(isOPEN){ // If this is the OPEN list
			  return s1.compareTo(s2); // This compares by costs
		  }
		  else{ // If this is the CLOSED list
			  StateComparator comparator = new StateComparator();
			  return comparator.compare(s1, s2); // This compares lexicographically
		  }
	  }
}
